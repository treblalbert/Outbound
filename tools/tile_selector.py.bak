...
BACKGROUND_SHEETS = {
    "Green (lush)": "assets/sprites/Tiles/Background_Green_TileSet.png",
    "Dark-Green (dry)": "assets/sprites/Tiles/Background_Dark-Green_TileSet.png",
    "Bleak-Yellow": "assets/sprites/Tiles/Background_Bleak-Yellow_TileSet.png",
}
BACKGROUND_COLS = 24
...
DEFAULTS = {
    ...
    "background": {
        ...
        "dirt": 39,
        "road": [221, 221],
        "grass_dirt": [5, 344, 342, 343, 296, 320, 368, 368, 294, 318, 318, 369, 295, 392, 393, -1],
    },
    ...
    "extras": {
        ...
        "brick_fill": [0, 1, 2, 3],
        "roof_fill": [0, 1, 2, 3, 16, 17, 18, 19],
        "interior_fill": [48, 50, 62, 64, 74, 76],
    },
    "sheet_overrides": {},
}
...
def cases():
    ...
    bg = BACKGROUND_SHEETS["Green (lush)"]
    dry = BACKGROUND_SHEETS["Dark-Green (dry)"]
    roof_sheet = "assets/sprites/Tiles/Roof_TileSet.png"
    fence_sheet = "assets/sprites/Tiles/Wire-Fence/Wire-Fence_TileSet.png"
    brick_sheet = "assets/sprites/Tiles/Brick-Wall_TileSet.png"
    garbage_sheet = "assets/sprites/Tiles/Garbage_TileSet.png"
    grass = list(range(5))

    # ---- terrain fills -----------------------------------------------------
    # The plain surfaces the outdoor renderer paints under everything else.
    add("Ground / grass", "grass variations (5 frames, picked by tile variant)",
        ("background", "grass"), bg, "background", indices=grass, count=5,
        ground=G_GRASS, mask_kind="grass_dirt")
    add("Ground / grass", "grass detail (3 clumps, used sparingly)",
        ("background", "grass_detail"), bg, "background", indices=[3, 51, 52], count=3,
        ground=G_GRASS, mask_kind="grass_dirt")
    add("Ground / bare earth", "bare earth",
        ("background", "dirt"), bg, "background", indices=[39], count=1,
        ground=G_DIRT, mask_kind="grass_dirt")
    add("Ground / bare earth", "sand, on the dry biome sheet",
        ("background", "sand"), dry, "background", indices=grass, count=5,
        ground=G_SAND, mask_kind="grass_dirt")
    add("Ground / asphalt", "asphalt (horizontal road / vertical road)",
        ("background", "road"), bg, "background", indices=[172, 197], count=2,
        ground=G_ROAD, mask_kind="grass_dirt")
    add("Ground / bridge & water", "bridge deck (falls back to asphalt)",
        ("extras", "bridge"), bg, "background", indices=[172], count=1,
        optional=True, ground=G_BRIDGE)
    add("Ground / bridge & water", "water (animated, never autotiled)",
        ("extras", "water"), bg, "background", indices=[172], count=1,
        optional=True, ground=G_WATER)
    add("Ground / grass-earth seam", "one frame per corner mask (clearable)",
        ("background", "grass_dirt"), bg, "background", indices=list(range(16)),
        count=16, clearable=True, ground=G_GRASS, mask_kind="grass_dirt")
    add("Ground / rubble", "debris left by a destroyed wall",
        ("extras", "rubble"), garbage_sheet, "background", indices=[0, 1, 2, 3],
        count=4, optional=True, ground=G_RUBBLE)
    ...
    for index, label in enumerate(("wood floor", "concrete floor", "tile floor",
                                   "base floor", "wood floor B", "concrete floor B",
                                   "tile floor B", "base floor B", "spare", "spare 2")):
        add("Buildings / floor", label, ("building", "floor", index), beige,
            "building", count=1, palette=True)
    add("Buildings / interior fallback", "interior fill when no floor frame is set",
        ("extras", "interior"), beige, "building", indices=[48, 50, 62, 64, 74, 76],
        count=6, palette=True, optional=True)

    # ---- walls -------------------------------------------------------------
    for index, label in enumerate(MASK_NAMES):
        add("Brick wall", f"mask {index:02d}: {label}",
            ("brick", "connected", index), brick_sheet, "brick", count=1,
            solid="brick", mask_kind="neighbour", mask=index)
    for index, label in enumerate(MASK_NAMES):
        add("Wire fence", f"mask {index:02d}: {label}",
            ("fence", "connected", index), fence_sheet, "fence", count=1,
        ...
        for row, row_name in enumerate(row_names):
            for col, col_name in enumerate(col_names):
                add(f"Roof / style {style + 1}", f"{row_name} / {col_name}",
                    ("roof", style, row, col), roof_sheet, "roof", count=1,
...
CASE_SHEETS = {
    ...
    "background": BACKGROUND_SHEETS["Green (lush)"],
}


class MapComposer:
    """Builds a preview map from the current mappings, using the renderer's rules.

    Every tile it paints is recorded in ``owners`` so a click on the map can be
    turned back into the case that produced it.  The layout is a small outdoor
    slice rather than a stand-in scene: grass and earth with a road and a dirt
    patch, a brick compound, a wire-fence pen, three roofed buildings (one with
    its roof fading off so the interior and floor frames are visible), rubble
    where a wall was destroyed, and every tree/rock/bush variant the pack has.

    Rules mirrored from src/art.cpp and src/game.cpp:

    * Ground autotiles through a 4 bit corner mask (1 NW, 2 NE, 4 SW, 8 SE)
      where the highest ``groundPriority`` surface at each shared corner wins,
      and only vegetation-versus-earth pairs draw seam art.
    * Walls mask their four orthogonal neighbours (1 left, 2 right, 4 up,
      8 down); a building perimeter adds 16/32/64/128 for its own edges so the
      facade frames win over the interior partition frames.
    * Roofs are two 5x3 blocks of style 1 and style 2, laid over the building
      footprint, and the roof over the tile you stand on fades out.
    """

    WIDTH = 52   # tiles
    HEIGHT = 46  # tiles; the bottom rows are a labelled reference gallery
    GALLERY_Y = 39
    ...
    def _sheet_path(self, group):
        ...
        if group == "background_dry":
            ...
            return ROOT / BACKGROUND_SHEETS["Dark-Green (dry)"]
        if override:
            return ROOT / override
        if group == "building":
    ...
    # -- the map ------------------------------------------------------------
    def compose(self):
        """Paint the preview map and return it, filling ``owners`` as it goes.

        Rows 0..GALLERY_Y-1 are a natural world slice (grass, earth, road,
        buildings, brick and fence runs, rubble), painted with the renderer's
        own rules. Rows GALLERY_Y..HEIGHT-1 are a labelled reference gallery:
        solid blocks of every case that can never show up in that world -- the
        sand frames on the dry sheet, the interior walls, the roof tiles the
        roof would hide, and the spare and fallback extras.
        """
    ...
    def _buildings(self):
        """(x0, y0, w, h, roof style, partition) for every preview building.

        ``partition`` is an interior wall stub drawn with the connected-wall
        frames, which is the only way the interior partition masks appear in a
        real world: facades take the perimeter cases first.
        """
        return [
            (6, 6, 9, 7, 0, (4, 10, 6)),      # a partition stub inside
            (25, 20, 8, 6, 1, None),
            (36, 5, 9, 7, 0, (40, 5, 3)),     # an L-shaped interior wall
            (4, 26, 9, 7, 1, None),
        ]

    def _in_building(self, x, y):
        for (bx, by, bw, bh, _s, _p) in self.buildings:
    ...
    def _paint_rubble(self, scene):
        """A stretch of wall blown away, leaving the rubble ground behind."""
        rubble = self._list(("extras", "rubble"), ())
        if not rubble:
            return
        case = self._case_for_list(("extras", "rubble"), 0)
        for (rx, ry) in self.rubble:
            frame = rubble[(rx + ry) % len(rubble)]
            scene.paste(self._tile("background", frame), (rx * TILE_SIZE, ry * TILE_SIZE))
            self.owners[(rx, ry)] = case
    ...
    def _paint_walls(self, scene):
        """Floors, facades, doors, interior partitions, brick and wire fence."""
        self.rubble = []
        for (bx, by, bw, bh, style, partition) in self.buildings:
            for y in range(by, by + bh):
                for x in range(bx, bx + bw):
                    edge = x in (bx, bx + bw - 1) or y in (by, by + bh - 1)
                    self.solid[y][x] = "facade" if edge else None
            if partition:
                px, py, plen = partition
                for i in range(plen):
                    if self.solid[py][px + i] is None:
                        self.solid[py][px + i] = "building"

        # A brick compound and a wire pen, so both wall families show their
        # straight runs, all four corners and a couple of T junctions.
        self._wall_run("brick", 2, 31, 12, "h")
        self._wall_run("brick", 2, 36, 12, "h")
        self._wall_run("brick", 2, 31, 6, "v")
        self._wall_run("brick", 13, 31, 6, "v")
        self._wall_run("brick", 6, 33, 3, "v")          # a T off the west wall
        self._wall_run("brick", 20, 31, 4, "v", 20, 6)  # isolated column
        self._wall_run("fence", 34, 26, 9, "v")
        self._wall_run("fence", 41, 26, 9, "v")
        self._wall_run("fence", 34, 34, 8, "h")
        self._wall_run("fence", 44, 28, 3, "v")         # a short spur

        # One stretch of wall blown away, leaving rubble on the ground.
        for i in range(4):
            x, y = 6 + i, 36
            if self.solid[y][x] == "brick":
                self.solid[y][x] = None
        ...
        for (x, y, kind) in self.wall_cells:
            if self.solid[y][x] is None:
                continue                                  # this one was blown up
            # A partition joins to the facade it meets, so both count as wall.
            same = {kind}
            if kind == "building":
                same.add("facade")
            mask = 0
            for (dx, dy, bit) in ((-1, 0, LEFT), (1, 0, RIGHT), (0, -1, UP), (0, 1, DOWN)):
                if self._wall_kind(x + dx, y + dy) in same:
                    mask |= bit
    ...
    def _wall_run(self, kind, x0, y0, length, axis, extra=-1, extra_length=0):
        ...
        for i in range(extra_length):
            if extra < 0:
                continue
            x, y = (extra + i, y0 + length - 1)
            if 0 <= x < self.WIDTH and 0 <= y < self.HEIGHT and self.solid[y][x] is None:
                self.solid[y][x] = "brick" if kind == "brick" else kind
                self.wall_cells.append((x, y, kind))

    def _paint_house(self, scene, bx, by, bw, bh):
        """One building: facade ring with corner-over-edge priority, floor inside.

        A gap is left in the south wall for the door, which is the one entrance
        the game draws as a real door sprite because no roof edge covers it.
        """
        facade = self._list(("building", "facade"), tuple(range(8)))
        floors = self._list(("building", "floor"), (0,))
        door = (bx + bw // 2, by + bh - 1)
        cells = self._house_wall_cells()
        for y in range(by, by + bh):
            for x in range(bx, bx + bw):
                if (x, y) == door:
                    self.owners[(x, y)] = self._case_for(("building", "facade"), 6)
                    continue
                ...
                if index is None:
                    ...
                    self.owners[(x, y)] = self._case_for_list(("building", "floor"), which)
                else:
    ...
    def _paint_roofs(self, scene):
        """Roofs last, so they hide the interior -- the renderer's own order.

        Each roof block is 5 wide by 3 tall of one style. A building 7 tiles or
        wider shows the left, middle and right columns of the block.
        """
        roof_data = self.data.get("roof", [])
        for (bx, by, bw, bh, style, _p) in self.buildings:
            block = roof_data[style % max(1, len(roof_data))] if roof_data else []
            for row in range(5):
                for col in range(3):
                    x = bx + (0 if bw < 3 else int((bw - 3) * col / 2))
                    y = by + (0 if bh < 5 else int((bh - 5) * row / 4))
                    frame = block[row][col] if row < len(block) and col < len(block[row]) else -1
                    piece = self._tile("roof", frame)
                    scene.alpha_composite(piece, (x * TILE_SIZE, y * TILE_SIZE))
...
class TileSelector(tk.Tk):
    def __init__(self):
        ...
        self.composer = SceneComposer(self.data, lambda: self.palette.get())
